...
class CoMotionDetect(nn.Module):
    ...
    def forward(self, img, K) -> DetectionOutput:
        ...
        #detections：一个包含所有预测的候选姿态参数
        #feature_pyramid：多尺度特征金字塔（参见笔记：FPN框架）

        for k, v in detections.items(): 
        #利用键值对提取所需值，其中有"betas" "delta_root_orient" "pose_embedding" "trans" "conf"
            if k == "pose_embedding":
                # Remap latent pose embedding to joint angles
                # Note: these are residual terms applied to a default pose
                outputs["appearance"] = v.clone()

                x_feat = self.pose_decoder[0](v)
                x_feat = self.pose_decoder[1](x_feat)   #使用升维操作将隐藏状态升到512维
                outputs["hidden"] = x_feat.clone()   #将隐藏状态传给Track
...
def decode_network_outputs(
    ...
    **nms_kwargs,
    ...
    detections = {
        ...
        "hidden": detect_out.hidden,    #增加了隐藏状态的传递
        "appearance": detect_out.appearance,    #增加了外貌特征的传递
    }
    ...
    #加入REID特征提取
    if img_tensor is not None and reid_model is not None:
        num_detected = len(nms_idxs)
        
        # 目标维度：DINOv2-Small 是 384
        target_dim = 384 
        device = detections["betas"].device

        if num_detected > 0:
            # A. 计算 Bounding Box
            # 此时 detections["pred_2d"] 已经是筛选过的，形状 (1, N, 27, 2)
            valid_pred_2d = detections["pred_2d"][0] # 取出 batch 维度 -> (N, 27, 2)
            
            # 计算关键点的最小和最大坐标
            min_xy = valid_pred_2d.min(dim=1)[0] # (N, 2)
            max_xy = valid_pred_2d.max(dim=1)[0] # (N, 2)
            
            # 拼接成 [x1, y1, x2, y2] 格式
            bboxes = torch.cat([min_xy, max_xy], dim=1) # (N, 4)
            
            # B. Padding (外扩)
            # 仅靠骨骼点裁剪会太紧凑，可能切掉头顶或衣服，加上 padding 效果更好
            pad = 25 
            bboxes[:, 0] -= pad # x1 左移
            bboxes[:, 1] -= pad # y1 上移
            bboxes[:, 2] += pad # x2 右移
            ...
            # C. 裁剪图片 & 提取特征
            # img_tensor 是 (1, 3, H, W)，我们需要取第 sample_idx 张图
            if img_tensor.ndim == 4:
                current_img = img_tensor[sample_idx]
            # 如果输入已经是 (3, H, W)，直接使用，不要切片！
            else:
                current_img = img_tensor
            
            # 调用 reid_module 中定义的静态方法裁剪
            crops = reid_model.crop_persons(current_img, bboxes)
            
            # 送入 DINOv2 提取特征 (N, 384)
            app_feats = reid_model(crops) 
            
            # D. 覆盖原有的 appearance
            # 增加 batch 维度 -> (1, N, 384)
            detections["appearance"] = app_feats.unsqueeze(0)
            
        else:
            # 如果没人，创建一个空的 (1, 0, 384) Tensor
            detections["appearance"] = torch.zeros((1, 0, target_dim), device=device)
    
    else:
        # 兼容逻辑：如果没有开启 ReID，我们也要确保 appearance 维度正确 (补零)
        # 否则 Tracker 那边拼接张量时会报错 (因为 Tracker 的 default_dims 已经改成 384 了)
        N = len(nms_idxs)
        target_dim = 384
        device = detections["betas"].device
        detections["appearance"] = torch.zeros((1, N, target_dim), device=device)
